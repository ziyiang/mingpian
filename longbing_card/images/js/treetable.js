layui.define(["jquery"], function (exports) {
        ...
        o.each(data, function (index, item) {
            ...
            o.each(t.c.cols, function (idx, obj) {
                ...
                if (obj.field == t.c.field) {
                    tbody += ("&nbsp;".repeat(level[item.id] * 8));
                    if (t.c.childs[item.id]) {
                        tbody += '<i class="layui-icon ' + t.c.icon_class + '">' + (item.id == t.cache(item.id) || t.c.is_open ? t.c.icon_val.close : t.c.icon_val.open) + "</i>"
                    }
                }
    ...
    tree.prototype.render = function (e) {
        ...
        o(t.c.elem).html(template).on("click", "td", function () {
            var id = o(this).parents("tr").data("tb-id"), pid = o(this).parents("tr").data("tb-pid"),
            ...
            return layui.event.call(this, MOD_NAME, t.c.events, {elem: o(this), item: t.c.new_data[id],})
        })
    };
    ...
    tree.prototype.cache = function (val, option) {
        var t = this, name = "tree-table-open-item", val = val.toString(),
            cache = t.get_cookie(name) ? t.get_cookie(name).split(",") : [], index = o.inArray(val, cache);
        if (option === undefined) {
